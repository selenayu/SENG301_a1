...
Added Scripts:
* T02-good-script1 should succeed. Edge cases tested in this script: when one or more coins/pops are loaded into the wrong chutes, press when there's not enough credit then more is added, unsorted coin chutes (not in ascending or descending order), extract after multiple transactions
* T03-good-script2 should succeed. Edge cases tested in this script: when the pop requested is sold out, when there's not enough change in the vending machine, unload vending machine with no coins in coin chutes and no pops, unload when there's money "in limbo"
